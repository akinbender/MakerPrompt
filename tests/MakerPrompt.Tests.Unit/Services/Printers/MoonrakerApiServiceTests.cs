...
public class MoonrakerApiServiceTests
{
    ...
    [Fact]
    public async Task GetFilesAsync_PreservesPrinterMetadata()
    {
        var service = new MoonrakerApiService(
            new FakeMoonrakerHandler(BuildDefaultResponses()));
        await service.ConnectAsync(BuildSettings());

        var files = await service.GetFilesAsync();

        var file = Assert.Single(files);
        Assert.Equal("gcodes/test.gcode", file.FullPath);
        Assert.Equal(1234, file.Size);
        Assert.True(file.IsAvailable);
        Assert.NotNull(file.ModifiedDate);
    }

    [Fact]
    public async Task GetPrinterQueueAsync_MapsQueuedJobs()
    {
        var handler = new FakeMoonrakerHandler(BuildDefaultResponses());
        var service = new MoonrakerApiService(handler);
        await service.ConnectAsync(BuildSettings());

        var queue = await service.GetPrinterQueueAsync();

        Assert.True(service.SupportsPrinterQueue);
        Assert.Contains(
            handler.RequestPaths,
            path => new Uri(path).AbsolutePath == "/server/job_queue/status");
        var job = Assert.Single(queue);
        Assert.Equal("job-42", job.JobId);
        Assert.Equal("queued.gcode", job.FileName);
        Assert.Equal(1_700_000_123, job.TimeAdded);
    }

    [Fact]
    public async Task ConnectAsync_AfterDisconnect_CreatesFreshConnectionLifetime()
    {
        var handler = new FakeMoonrakerHandler(BuildDefaultResponses());
        var service = new MoonrakerApiService(handler);
        ...
        var reconnected = await service.ConnectAsync(BuildSettings());

        Assert.True(reconnected);
        Assert.True(service.IsConnected);
        Assert.Equal(
            2,
            handler.RequestPaths.Count(path =>
                new Uri(path).AbsolutePath == "/printer/info"));
    }
    ...
    private static Func<HttpRequestMessage, HttpResponseMessage> BuildDefaultResponses()
    {
        return request =>
        {
            ...
            return path switch
            {
                ...
                "/server/job_queue/status" => JsonResponse(
                    """{"queued_jobs":[{"job_id":"job-42","filename":"queued.gcode","time_added":1700000123}],"queue_state":"ready"}"""),
    ...
    private static PrinterConnectionSettings BuildSettings() =>
        new() { ConnectionType = PrinterConnectionType.Moonraker, ApiUrl = "http://moonraker.local" };
