...
public class PrusaLinkApiServiceTests
{
    ...
    public async Task ConnectAsync_UsesVersionAndInfoEndpoints()
    {
        ...
        Assert.False(service.SupportsCommandPrompt);
    ...
    public async Task GetFilesAsync_ReturnsChildrenFromDefaultStorage()
    {
        ...
        Assert.NotNull(files[0].ModifiedDate);
    }
    ...
    public async Task ConnectAsync_AfterDisconnect_ReusesAdapterSafely()
    {
        ...
        var service = new PrusaLinkApiService(new FakePrusaLinkHandler(request =>
        {
    ...
    [Fact]
    public async Task ConnectAsync_RetriesTransientVersionFailureWithFreshRequests()
    {
        var attempts = 0;
        var defaultResponder = BuildDefaultResponses();
        var service = new PrusaLinkApiService(new FakePrusaLinkHandler(request =>
        {
            if (request.RequestUri?.AbsolutePath == "/api/version" && ++attempts < 3)
    ...
    private static PrinterConnectionSettings BuildSettings() =>
        new() { ConnectionType = PrinterConnectionType.PrusaLink, ApiUrl = "http://printer.local", UserName = "user", Password = "pass" };
