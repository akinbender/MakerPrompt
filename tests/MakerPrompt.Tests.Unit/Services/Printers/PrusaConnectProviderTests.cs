...
public class PrusaConnectProviderTests
{
    ...
    public async Task GetPrintersAsync_ReturnsMapping_FromRootArray()
    {
        ...
        await provider.ConfigureAsync(TestToken);
    ...
    {
        // No ConfigureAsync() call → no auth header → 401 from real server.
    ...
    [Fact]
    public async Task GetPrintersAsync_MapsNestedPrinterState()
    {
        const string json = """
            [
              {
                ...
                "name":"Nested",
                "printer_type":"XL",
                "printer_state":{"text":"PAUSED"}
              }
            ]
            """;
        var provider = new PrusaConnectProvider(
            new FakeHandler(_ => JsonResponse(json)));
        await provider.ConfigureAsync(TestToken);

        var printer = Assert.Single(await provider.GetPrintersAsync());

        Assert.Equal("PAUSED", printer.RawStatus);
        Assert.Equal(PrinterStatus.Paused, printer.Status);
    }
