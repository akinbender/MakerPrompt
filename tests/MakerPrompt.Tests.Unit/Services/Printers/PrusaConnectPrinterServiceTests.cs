...
public class PrusaConnectPrinterServiceTests
{
    ...
    public async Task ConnectAsync_SucceedsWithValidPrinterResponse()
    {
        ...
        Assert.False(service.SupportsDirectControl);
        Assert.False(service.SupportsPrintStart);
    }
    ...
    public async Task WriteDataAsync_PostsCommandToCommandEndpoint()
    {
        ...
        var defaultResponder = DefaultResponder();
    ...
    [Fact]
    public async Task GetTelemetryAsync_HandlesAlternateMobileResponseShapes()
    {
        var service = new PrusaConnectPrinterService(new FakeHandler(request =>
        {
            var path = request.RequestUri?.AbsolutePath;
            if (path == $"/api/v1/printers/{TestUuid}/telemetry")
            {
                return JsonResponse(
                    """{"temp_nozzle":"205.5","axis_z":"12.75","unmapped_telemetry":true}""");
            }
                ...
                return JsonResponse($$"""
                    {
                      ...
                      "printer_state":{"text":"PRINTING"},
                      "job":{"progress":42.5,"time_printing":120,"file_name":"alternate.gcode"},
                      "unmapped_printer":"kept-for-diagnostics"
                    }
                    """);
            }
        ...
        Assert.Equal(PrinterStatus.Printing, telemetry.Status);
        Assert.Equal(12.75f, telemetry.Position.Z);
        Assert.Equal(42.5, telemetry.SDCard.Progress);
        Assert.Equal("alternate.gcode", telemetry.PrintJobName);
    }
    ...
    public async Task ConnectAsync_AfterDisconnect_ReusesAdapterSafely()
    {
        ...
        var service = new PrusaConnectPrinterService(new FakeHandler(request =>
        {
            if (request.RequestUri?.AbsolutePath == $"/api/v1/printers/{TestUuid}")
                requestCount++;
    ...
    [Fact]
    public async Task ConnectAsync_AcceptsLegacyUsernamePrinterId()
    {
        var service = new PrusaConnectPrinterService(new FakeHandler(DefaultResponder()));
        var legacySettings = new PrinterConnectionSettings
        {
        ...
        Assert.True(await service.ConnectAsync(legacySettings));
    }
