...
public sealed class PrinterFleetServiceTests : IAsyncDisposable
{
    ...
    [Fact]
    public async Task AddAndConnect_FailedConnect_ReturnsFalseButPrinterStillRegistered()
    {
        ...
        Assert.False(result);
        // Service is registered even on connection failure so the caller can retry.
        Assert.Contains("p1", _sut.PrinterIds);
    }
