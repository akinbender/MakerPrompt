...
public sealed class SqliteStoreTests
{
    ...
    [Fact]
    public async Task TelemetryStore_Save_PrunesHistoryPerPrinter()
    {
        using var db = CreateTempDb();
        await using var store = new SqliteTelemetryStore(
            db.ConnectionString,
            NullLogger<SqliteTelemetryStore>.Instance,
            maxSnapshotsPerPrinter: 2);

        for (var index = 0; index < 3; index++)
        {
            await store.SaveAsync("p-retained", new PrinterTelemetry
            {
                HotendTemp = index,
        ...
        var history = await store.GetHistoryAsync("p-retained", count: 10);

        Assert.Equal(2, history.Count);
        Assert.Equal(2, history[0].HotendTemp);
    }
    ...
    [Fact]
    public async Task CameraStore_Save_PrunesHistoryPerCamera()
    {
        using var db = CreateTempDb();
        await using var store = new SqliteCameraSnapshotStore(
            db.ConnectionString,
            NullLogger<SqliteCameraSnapshotStore>.Instance,
            maxSnapshotsPerCamera: 2);

        for (var index = 0; index < 3; index++)
        {
            await store.SaveAsync(new CameraSnapshot
            {
                CameraId = "cam-retained",
                Label = $"Frame-{index}",
                JpegData = [0xFF, 0xD8, (byte)index, 0xFF, 0xD9],
                CapturedAt = DateTimeOffset.UtcNow.AddSeconds(index),
            });
        }

        var history = await store.GetHistoryAsync("cam-retained", count: 10);

        Assert.Equal(2, history.Count);
        Assert.Equal("Frame-2", history[0].Label);
    }
}
