...
public sealed class InMemoryTelemetryStoreTests
{
    ...
    [Fact]
    public async Task Save_CapturesSnapshotInsteadOfMutableReference()
    {
        ...
        await _sut.SaveAsync("p1", telemetry);
        telemetry.HotendTemp = 250;
        telemetry.PrintProgress = 90;

        var stored = await _sut.GetLatestAsync("p1");

        Assert.Equal(200, stored!.HotendTemp);
        Assert.Equal(10, stored.PrintProgress);
    }
}
