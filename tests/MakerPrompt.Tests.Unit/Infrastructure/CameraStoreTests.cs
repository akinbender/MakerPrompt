...
public sealed class CameraStoreTests
{
    ...
    {
        var store = new InMemoryCameraSnapshotStore();
        await store.SaveAsync(new CameraSnapshot { CameraId = "camX", Label = "X", JpegData = [1] });
        await store.SaveAsync(new CameraSnapshot { CameraId = "camY", Label = "Y", JpegData = [2] });
    ...
    [Fact]
    public async Task InMemoryCameraStore_Save_CopiesJpegBytes()
    {
        ...
        var stored = await store.GetLatestAsync("camera");

        Assert.Equal(new byte[] { 1, 2, 3 }, stored!.JpegData);
    }
}
