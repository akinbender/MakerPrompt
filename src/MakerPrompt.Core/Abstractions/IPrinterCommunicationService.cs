...
public interface IPrinterCommunicationService : IAsyncDisposable
{
    ...
    /// <summary>
    /// <c>true</c> when this backend supports direct printer controls such as
    /// temperature changes, homing, movement, extrusion, fan, speed, and flow.
    /// Monitoring-only backends should return <c>false</c>.
    /// </summary>
    ...
    /// <summary>
    /// <c>true</c> when this backend can start a print from a file or streamed
    /// G-code document. Read-only cloud backends should return <c>false</c>.
    /// </summary>
    bool SupportsPrintStart => true;

    /// <summary>
    /// <c>true</c> when this backend exposes a queryable printer-side queue.
    /// </summary>
    ...
    /// <summary>Returns files available on the printer, including storage metadata.</summary>
    Task<IReadOnlyList<FileEntry>> GetFilesAsync(CancellationToken cancellationToken = default);

    /// <summary>
    /// Returns printer-side queued jobs. Backends without queue support return
    /// an empty list and expose <see cref="SupportsPrinterQueue"/> as <c>false</c>.
    /// </summary>
    Task<IReadOnlyList<PrintQueueEntry>> GetPrinterQueueAsync(
        CancellationToken cancellationToken = default) =>
        Task.FromResult<IReadOnlyList<PrintQueueEntry>>([]);
    ...
    /// <summary>Sets firmware steps-per-unit values for one or more axes.</summary>
    Task SetAxisPerUnitAsync(float x = 0f, float y = 0f, float z = 0f, float e = 0f,
        CancellationToken cancellationToken = default);

    /// <summary>Runs firmware PID tuning for the selected extruder.</summary>
    Task RunPidTuningAsync(int cycles, int targetTemp, int extruderIndex,
        CancellationToken cancellationToken = default);

    /// <summary>Runs firmware thermal-model calibration.</summary>
    Task RunThermalModelCalibrationAsync(int cycles, int targetTemp,
        CancellationToken cancellationToken = default);
    ...
    /// <summary>Persists firmware settings to non-volatile storage.</summary>
    Task SaveEepromAsync(CancellationToken cancellationToken = default);
}
