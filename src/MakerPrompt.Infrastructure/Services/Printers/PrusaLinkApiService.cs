...
public class PrusaLinkApiService : BasePrinterConnectionService, IPrinterCommunicationService
{
    ...
    public async Task<bool> ConnectAsync(PrinterConnectionSettings connectionSettings, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            var version = await GetVersionAsync(linkedCts.Token);
            if (version == null)
            {
                IsConnected = false;
            ...
            var info = await GetInfoAsync(linkedCts.Token);
    ...
    public async Task<IReadOnlyList<FileEntry>> GetFilesAsync(CancellationToken cancellationToken = default)
    {
        using var linkedCts =
            CancellationTokenSource.CreateLinkedTokenSource(_cts.Token, cancellationToken);
        var storages = await GetStorageAsync(linkedCts.Token);
        ...
        var folder = await GetFolderAsync(storageKey, "/", linkedCts.Token);
...
#pragma warning restore CA1416
        ...
        string path, CancellationToken cancellationToken)
    {
        ...
        throw new HttpRequestException($"Request to '{path}' failed after {maxAttempts} attempts.");
    }
