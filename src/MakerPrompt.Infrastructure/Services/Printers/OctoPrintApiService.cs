...
public sealed class OctoPrintApiService : BasePrinterConnectionService, IPrinterCommunicationService
{
    ...
    public async Task<bool> ConnectAsync(PrinterConnectionSettings connectionSettings, CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            using var response = await Client.GetAsync(
                "/api/version", linkedCts.Token).ConfigureAwait(false);
            ...
            var versionJson = await response.Content
                .ReadAsStringAsync(linkedCts.Token).ConfigureAwait(false);
            ...
            using var stateResponse = await Client.GetAsync(
                "/api/connection", linkedCts.Token).ConfigureAwait(false);
            if (stateResponse.IsSuccessStatusCode)
            {
                var stateJson = await stateResponse.Content
                    .ReadAsStringAsync(linkedCts.Token).ConfigureAwait(false);
                    ...
                    current.TryGetProperty("state", out var stateStr))
                {
                    ...
                    {
                        await SendConnectCommandAsync(linkedCts.Token).ConfigureAwait(false);
                    }
                }
            }
    ...
    /// </summary>
    private async Task SendConnectCommandAsync(CancellationToken cancellationToken)
    {
        try
        {
            ...
            using var response = await Client.PostAsync(
                "/api/connection", content, cancellationToken).ConfigureAwait(false);
        }
        catch (OperationCanceledException) when (cancellationToken.IsCancellationRequested)
        {
            throw;
    ...
    public async Task WriteDataAsync(string command, CancellationToken cancellationToken = default)
    {
        ...
        using var linkedCts =
            CancellationTokenSource.CreateLinkedTokenSource(_cts.Token, cancellationToken);
        using var response = await Client.PostAsync(
            "/api/printer/command", content, linkedCts.Token).ConfigureAwait(false);
    ...
    /// </summary>
    private async Task SendGcodeAsync(
        CancellationToken cancellationToken, params string[] commands)
    {
        if (!IsConnected) return;
    ...
    public async Task<PrinterTelemetry> GetTelemetryAsync(CancellationToken cancellationToken = default)
    {
        ...
        try
        {
            ...
            using var printerResponse = await Client.GetAsync(
                "/api/printer", linkedCts.Token).ConfigureAwait(false);
            if (printerResponse.IsSuccessStatusCode)
            {
                var json = await printerResponse.Content
                    .ReadAsStringAsync(linkedCts.Token).ConfigureAwait(false);
            ...
            using var jobResponse = await Client.GetAsync(
                "/api/job", linkedCts.Token).ConfigureAwait(false);
            if (jobResponse.IsSuccessStatusCode)
            {
                var json = await jobResponse.Content
                    .ReadAsStringAsync(linkedCts.Token).ConfigureAwait(false);
                ParseJobState(json);
            }
        }
    ...
    public async Task<IReadOnlyList<FileEntry>> GetFilesAsync(CancellationToken cancellationToken = default)
    {
        ...
        {
            using var response = await Client.GetAsync(
                "/api/files?recursive=true", linkedCts.Token).ConfigureAwait(false);
    ...
    public Task SetHotendTempAsync(int targetTemp = 0, CancellationToken cancellationToken = default) =>
        SendGcodeAsync(cancellationToken, $"M104 S{targetTemp}");

    public Task SetBedTempAsync(int targetTemp = 0, CancellationToken cancellationToken = default) =>
        SendGcodeAsync(cancellationToken, $"M140 S{targetTemp}");
    ...
    public Task HomeAsync(bool x = true, bool y = true, bool z = true, CancellationToken cancellationToken = default)
    {
        ...
        if (y) axes.Append(" Y");
        if (z) axes.Append(" Z");
        return SendGcodeAsync(cancellationToken, axes.ToString());
    }
    ...
    public Task RelativeMoveAsync(int feedRate, float x = 0.0f, float y = 0.0f, float z = 0.0f, float e = 0.0f, CancellationToken cancellationToken = default)
    {
        ...
        return SendGcodeAsync(cancellationToken, sb.ToString().Split('\n'));
    }
    ...
    public Task SetFanSpeedAsync(int fanSpeedPercentage = 0, CancellationToken cancellationToken = default)
    {
        ...
        return SendGcodeAsync(cancellationToken, $"M106 S{duty}");
    }

    public Task SetPrintSpeedAsync(int speed, CancellationToken cancellationToken = default) =>
        SendGcodeAsync(cancellationToken, $"M220 S{Math.Clamp(speed, 1, 200)}");

    public Task SetPrintFlowAsync(int flow, CancellationToken cancellationToken = default) =>
        SendGcodeAsync(cancellationToken, $"M221 S{Math.Clamp(flow, 1, 200)}");
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        return SendGcodeAsync(cancellationToken, sb.ToString());
    }
    ...
    public Task RunPidTuningAsync(
        ...
        CancellationToken cancellationToken = default) =>
        SendGcodeAsync(
            cancellationToken, $"M303 E{extruderIndex} S{targetTemp} C{cycles}");

    public Task RunThermalModelCalibrationAsync(
        int cycles, int targetTemp, CancellationToken cancellationToken = default) =>
        SendGcodeAsync(cancellationToken, $"M303 E-1 S{targetTemp} C{cycles}");
    ...
    public async Task StartPrintAsync(string fileName, CancellationToken cancellationToken = default)
    {
        ...
        using var linkedCts =
            CancellationTokenSource.CreateLinkedTokenSource(_cts.Token, cancellationToken);
        using var response = await Client.PostAsync(
            $"/api/files/{location}/{filePath}", content, linkedCts.Token).ConfigureAwait(false);
        response.EnsureSuccessStatusCode();
    }
        ...
        GCodeDoc gcodeDoc, CancellationToken cancellationToken = default)
    {
        ...
        await foreach (var command in gcodeDoc.EnumerateCommandsAsync(linkedCts.Token))
        {
            ...
            await SendGcodeAsync(linkedCts.Token, command);
        }
    }

    public Task SaveEepromAsync(CancellationToken cancellationToken = default) =>
        SendGcodeAsync(cancellationToken, "M500");
